...
namespace BudgetAppCross.Core.Services
{
    public sealed class SQLiteDataReader : DbDataReader
    {
        ...
        Sqlite3Statement m_currentStatement;
        bool m_hasRead;
        DbType?[] m_columnType;
        ...
        private Sqlite3DatabaseHandle DatabaseHandle => ((XamarinSQLiteConnection)m_command.Connection).Handle;
        ...
        public override int FieldCount
        {
            ...
            {
                VerifyNotDisposed();
                return m_hasRead ? m_columnType.Length : SQLiteIvie.ColumnCount(m_currentStatement);
            }
        }
        ...
        public override string GetName(int ordinal)
        {
            ...
            return SQLiteIvie.ColumnName(m_currentStatement, ordinal);
        }
        ...
        public override int GetOrdinal(string name)
        {
            ...
            if (m_columnNames == null)
            {
                ...
                {
                    var columnName = SQLiteIvie.ColumnName(m_currentStatement, i);
        ...
        private Task<bool> NextResultAsyncCore(CancellationToken cancellationToken)
        {
            ...
            try
            {
                for (int i = 0; i < m_command.Parameters.Count; i++)
                {
                    ...
                    if (parameterName != null)
                    {
                        ...
                        index = SQLiteIvie.BindParameterIndex(m_currentStatement, parameterName);
                    }
                    else
                    {
                        index = i + 1;
                    ...
                    if (index > 0)
                    {
                        ...
                        {
                            ThrowOnError(SQLiteIvie.BindNull(m_currentStatement, index));
                        }
                        else if (value is int || (value is Enum && Enum.GetUnderlyingType(value.GetType()) == typeof(int)))
                        {
                            ThrowOnError(SQLiteIvie.BindInt(m_currentStatement, index, (int)value));
                        }
                        else if (value is bool)
                        {
                            ThrowOnError(SQLiteIvie.BindInt(m_currentStatement, index, ((bool)value) ? 1 : 0));
                        }                        
                        else if (value is string)
                        {
                            SQLiteIvie.BindText(m_currentStatement, index, (string)value, -1, NegativePointer);
                        }
                        else if (value is byte[])
                        {
                            SQLiteIvie.BindBlob(m_currentStatement, index, (byte[])value, -1, NegativePointer);
                        }
                        else if (value is long)
                        {
                            ThrowOnError(SQLiteIvie.BindInt64(m_currentStatement, index, (long)value));
                        }
                        else if (value is float)
                        {
                            ThrowOnError(SQLiteIvie.BindDouble(m_currentStatement, index, (float)value));
                        }
                        else if (value is double)
                        {
                            ThrowOnError(SQLiteIvie.BindDouble(m_currentStatement, index, (double)value));
                        }
                        else if (value is DateTime)
                        {
                            SQLiteIvie.BindText(m_currentStatement, index, ToString((DateTime)value), -1, NegativePointer);
                        }
                        else if (value is Guid)
                        {
                            SQLiteIvie.BindBlob(m_currentStatement, index, ((Guid)value).ToByteArray(), -1, NegativePointer);
                        }
                        else if (value is byte)
                        {
                            ThrowOnError(SQLiteIvie.BindInt(m_currentStatement, index, (byte)value));
                        }
                        else if (value is short)
                        {
                            ThrowOnError(SQLiteIvie.BindInt(m_currentStatement, index, (short)value));
                        }
                        else
                        {
                            SQLiteIvie.BindText(m_currentStatement, index, Convert.ToString(value, CultureInfo.InvariantCulture), -1, NegativePointer);
                        }
                    }
                }
                success = true;
            ...
            finally
            {
                if (!success)
                {
                    ThrowOnError(SQLiteIvie.Reset(m_currentStatement));
                }
            }
        ...
        private Task<bool> ReadAsyncCore(CancellationToken cancellationToken)
        {
            using (cancellationToken.Register(s_interrupt, DatabaseHandle, useSynchronizationContext: false))
            {
                ...
                {
                    SQLiteIvie.Result errorCode = SQLiteIvie.Step(m_currentStatement);

                    switch (errorCode)
                    {
                        case SQLiteIvie.Result.Done:
                            Reset();
                            return s_falseTask;

                        case SQLiteIvie.Result.Row:
                            m_hasRead = true;
                            if (m_columnType == null)
                                m_columnType = new DbType?[SQLiteIvie.ColumnCount(m_currentStatement)];
                            return s_trueTask;
                        ...
                        case SQLiteIvie.Result.Interrupt:
                            return s_canceledTask;

                        default:
                            throw SQLiteException.New(errorCode, SQLiteIvie.GetErrmsg(DatabaseHandle));
                    }
                }
            }
        ...
        private static readonly Action<object> s_interrupt = obj => SQLiteIvie.Interrupt((Sqlite3DatabaseHandle)obj);

        private void Reset()
        {
            if (m_currentStatement != null)
            {
                SQLiteIvie.Reset(m_currentStatement);
            }
        ...
        private void ThrowOnError(SQLiteIvie.Result errorCode)
        {
            if (errorCode != SQLiteIvie.Result.OK)
                throw SQLiteException.New(errorCode, SQLiteIvie.GetErrmsg(DatabaseHandle));
        }
        ...
        private string ToString(DateTime dateTime)
        {
            ...
            string formatString = "yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'fff";
